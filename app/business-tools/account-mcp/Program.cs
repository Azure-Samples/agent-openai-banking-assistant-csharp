...
var loggerConfig = new LoggerConfiguration()
    ...
    .Enrich.WithThreadId()
    .Enrich.WithProperty("Application", "AccountMcp")
...
if (appInsightsActive)
{
    ...
    if (!string.IsNullOrEmpty(appInsightsConnectionString))
    {
        var resourceBuilder = ResourceBuilder
            .CreateDefault()
            .AddService("AccountMcp");

        builder.Services.AddOpenTelemetry()
            .WithTracing(tracing => tracing
                .SetResourceBuilder(resourceBuilder)
                .AddSource("AccountMcp.Tools.AccountTool")
                .AddSource("AccountMcp.Tools.UserTool")
...
try
{
    Log.Information("Starting Account MCP Server");
...
{
    Log.Fatal(ex, "Account MCP Server terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
